...
public class MotorAdapterTest {
    ...
    public static void setup() {
        Model adapterTBOX = EasyMock.createMock(Model.class);
        ResIterator resIterator = EasyMock.createMock(ResIterator.class);
        EasyMock.expect(resIterator.hasNext()).andReturn(true).once();
        EasyMock.expect(resIterator.nextResource()).andReturn(MessageBusOntologyModel.classAdapter).anyTimes();
        EasyMock.expect(adapterTBOX.listResourcesWithProperty(RDFS.subClassOf, MessageBusOntologyModel.classAdapter)).andReturn(resIterator).anyTimes();
        Property longP = EasyMock.createMock(Property.class);
        Property latP = EasyMock.createMock(Property.class);

        EasyMock.expect(adapterTBOX.createProperty("http://www.w3.org/2003/01/geo/wgs84_pos#long")).andReturn(longP).anyTimes();
        EasyMock.expect(adapterTBOX.createProperty("http://www.w3.org/2003/01/geo/wgs84_pos#lat")).andReturn(latP).anyTimes();

        Resource adapterABox = EasyMock.createMock(Resource.class);
        EasyMock.expect(adapterABox.getNameSpace()).andReturn("http://localhost").anyTimes();
        EasyMock.expect(adapterABox.addProperty(EasyMock.anyObject(Property.class),EasyMock.anyObject(Resource.class))).andReturn(adapterABox).anyTimes();
        EasyMock.expect(adapterABox.addProperty(EasyMock.anyObject(Property.class),EasyMock.anyObject(String.class))).andReturn(adapterABox).anyTimes();
        EasyMock.expect(adapterABox.getLocalName()).andReturn("InstanceOne").anyTimes();
        EasyMock.expect(adapterABox.addLiteral(EasyMock.anyObject(Property.class),EasyMock.anyInt())).andReturn(adapterABox).anyTimes();

        StmtIterator stmtIterator = EasyMock.createMock(StmtIterator.class);
        EasyMock.expect(stmtIterator.hasNext()).andReturn(true).once();
        Statement statement = EasyMock.createMock(Statement.class);
        RDFNode rdfNode = EasyMock.createMock(RDFNode.class);
        ...
        EasyMock.expect(adapterABox.listProperties(EasyMock.anyObject(Property.class))).andReturn(stmtIterator).anyTimes();
        EasyMock.expect(adapterTBOX.listSubjectsWithProperty(RDFS.domain,resource)).andReturn(EasyMock.createMock(ResIterator.class));
        EasyMock.expect(adapterTBOX.listStatements(EasyMock.anyObject(Resource.class),EasyMock.anyObject(Property.class),EasyMock.anyObject(RDFNode.class))).andReturn(EasyMock.createMock(StmtIterator.class)).anyTimes();
        EasyMock.replay(adapterABox, adapterTBOX, resIterator,stmtIterator,statement,rdfNode);
        adapter = new MotorAdapter(adapterTBOX,adapterABox);
    }

    @Test
    public void testCreateAndTerminate() throws InstanceNotFoundException, ProcessingException, InvalidRequestException {
        String instanceURI = adapter.getAdapterABox().getNameSpace()+"InstanceOne";
