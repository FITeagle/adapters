...
public class SSHConnector {
	...
	public SSHConnector(String ip, String port, String username,
	...
	public void createUserAccount(String newUser) {
		try {
			...
			try {
				Command cmd = session.exec("echo " + password
	...
	public void changeOwnerOfUserHome(String newUser) {
		try {
			...
			try {
				...
				Command com2 = session.exec("echo " + password
						+ "| sudo -S chown -R " + newUser + ":" + newUser
						+ " /home/" + newUser + "/.ssh");
				System.out.println(IOUtils.readFully(com2.getInputStream())
						.toString());
	...
	public void deleteUser(String newUser) {
		try {
			...
			try {
				...
				Command command = session.exec("echo " + password
						+ "| sudo -S userdel -r " + newUser + "");
