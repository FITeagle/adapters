...
public class OpenBatonClient {
	...
	// checkRequestor();
	// NetworkServiceDescriptorRestAgent nsdAgend =
	// nfvoRequestor.getNetworkServiceDescriptorAgent();
	...
	// checkRequestor();
	// VirtualNetworkFunctionDescriptorRestAgent vnfAgent =
	// nfvoRequestor.getVirtualNetworkFunctionDescriptorRestAgent();
	// try {
	...
	public NetworkServiceRecord updateNetworkServiceRecord(NetworkServiceRecord nsr) {
		this.checkRequestor();
		try {
			NetworkServiceRecord updatedNSR = (NetworkServiceRecord) this.nfvoRequestor.getNetworkServiceRecordAgent()
					.findById(nsr.getId());
			return updatedNSR;
		} catch (SDKException e) {
			e.printStackTrace();
	...
	public void uploadPackageToDatabase(String fileNameWithDirectory) {
		checkRequestor();
		try {
			VNFPackageAgent agent = nfvoRequestor.getVNFPackageAgent();
			agent.create(fileNameWithDirectory);
		} catch (SDKException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
